...
const getCommentsByArticleId = (req, res) => {
  const article_id = req.params.id;
  const query = `SELECT c.comment,c.article_id,u.firstName, c.commenter_id FROM comments c JOIN users u ON u.id=c.commenter_id WHERE c.article_id=$1 AND c.is_deleted=0;`;
